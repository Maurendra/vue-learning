...
<div>
  <div className="max-w-6xl mx-auto py-8">
    <div @click="toggleModalAdd" className="bg-blue-400 hover:bg-blue-800 text-white border-2 border-white rounded-lg py-2 px-4 cursor-pointer w-fit mb-4">
  ...
  methods: {
    async getEvents() {
      ...
      if (res.data) {
        ...
        let data = [];
        ev.map((event, index) => {
          if (event.company_id == this.auth.id) {
            data.push(event);
          }
        });
        if (data.length > 0) {
